...
const EditDocumentForm = ({ document, onSave, onCancel, isLoading }) => {
  ...
  const content = (
    <div className='modal-overlay' onClick={(e) => {
      ...
      <div style={{ background: '#fff', borderRadius: 8, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 30px rgba(0,0,0,0.2)', maxHeight: '92vh', width: 'min(900px, 100%)' }}>
        <div className={`edit-document-form document-confirmation-form${isInvoice ? ' invoice' : ''}`} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <form onSubmit={handleSubmit} className="document-form" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' }}>
            <div className="modal-body modal-body--scrollable" style={{ flex: 1, minHeight: 0, paddingBottom: 0, overflowY: 'auto' }}>
  ...
  return typeof window !== 'undefined' ? createPortal(content, window.document.body) : content;
};
